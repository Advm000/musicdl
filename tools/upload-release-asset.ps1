# Upload robuste d'un asset de release GitHub (HTTP/1.1, retries, vérification d'état)
...
for ($try = 1; $try -le 3; $try++) {
  ...
  # 1. Supprimer l'asset existant du même nom (cassé ou partiel)
  ...
  if ($old) {
    ...
    Write-Host "Ancien asset supprimé (state=$($old.state))"
    Start-Sleep 3
  }
  ...
  & "$env:SystemRoot\System32\curl.exe" --http1.1 -sS -X POST `
    ...
    -o "$env:TEMP\gh-upload-out.json" -w "curl: HTTP %{http_code}, %{size_upload} octets, %{time_total}s`n"
  # 3. Vérifier l'état final côté API
  Start-Sleep 4
  $assets = Invoke-RestMethod -Uri "https://api.github.com/repos/$Repo/releases/$ReleaseId/assets" -Headers $h
  ...
  Write-Host "Etat après tentative: $(if ($a) { $a.state } else { 'absent' }) — on réessaie"
}
Write-Host "ECHEC après 3 tentatives"
exit 1
